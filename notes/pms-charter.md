...
- 2026-06-04 (Lord-Shipwright) — **Phase 3 Publish flow WIRED + Publish button SHIPPED. One GitHub PAT permission fix left for Araya.**
  - **systemd env wiring (durable):** Araya installed the PAT at `/etc/pocketbase.env` (root-only `0600`, `GH_DISPATCH_PAT=…`). I created a systemd drop-in `/etc/systemd/system/pocketbase.service.d/10-pms-pat.conf` = `[Service]\nEnvironmentFile=/etc/pocketbase.env`, `daemon-reload` + `restart`. Verified: service `active`, `systemctl show` lists the drop-in + EnvironmentFile, the env var IS present in the live process (`/proc/<pid>/environ`, key-only check — value never printed), and it **persists across restart**. `/pb/api/health` = 200. The 503 "publish not configured" is GONE — the hook now reads the token and calls GitHub.
  - **⚠ PAT PERMISSION FINDING (the one blocker):** the hook now reaches GitHub but the dispatch is rejected **403 "Resource not accessible by personal access token."** GitHub's own response header on the dispatch endpoint says `x-accepted-github-permissions: contents=write`. The installed fine-grained PAT can READ the repo (admin/push/maintain all true, expires 2026-09-02) but lacks **Contents: Read and write**. **The earlier charter note saying "Actions: read/write" was wrong for this endpoint** — `POST /repos/{o}/{r}/dispatches` requires **Contents: write**, not Actions. **Araya's fix:** github.com → Settings → Developer settings → Fine-grained tokens → edit the existing token (or regenerate) → set **Contents: Read and write** for `Digital360-Indonesia/rework-kg`. If GitHub forces a regenerate, paste the new value into `/etc/pocketbase.env` (replace the line) and `sudo systemctl restart pocketbase` — no other change needed. After that, Publish works with zero further wiring.
  - **Publish button SHIPPED:** new hook `/home/fabrikgroup/pocketbase/pb_hooks/publish_page.pb.js` (owned `fabrikgroup`, 644) adds `GET /publish` → serves a small branded page (KG navy + orange, Indonesian label **"Terbitkan ke situs live"**). It reads the logged-in superuser's token from `localStorage["pocketbase_auth"].token` (the session set by `/pb/_/`) and POSTs to `/pms-publish`, showing "Build dimulai!" on success or the precise error (401 → "login lagi", else the GitHub error). No password stored in the page. **URL for Araya: `https://dev.kustomgarment.com/pb/publish`** (must be logged into `/pb/_/` in the same browser first). Verified serving 200 on origin + public URL.
  - **End-to-end run:** NOT yet completed — blocked solely on the PAT Contents:write fix above. Everything else is proven (env loaded, hook calls GitHub, `deploy.yml` on `main` has `repository_dispatch:[pms-publish]` + `PMS_SOURCE: pb` — both confirmed live via API). The moment the token permission is bumped, clicking Publish (or a superuser POST to `/pms-publish`) returns `{ok:true,dispatched:"pms-publish"}` and fires a deploy run.
  - **Integrity:** temp test superuser `pms-pubtest@kustomgarment.local` created for the curl test then **deleted** (auth now 400). `_superusers` back to 1. `briefs` = 12, untouched. No other tenant/config touched; PocketBase lives entirely under `/home/fabrikgroup/pocketbase/` (not HestiaCP-templated). Both hooks survive restart/regen.

- 2026-06-04 (Manager-Lord) — **PUBLISH FLOW FULLY LIVE.** GH PAT installed by Araya on the box (`/etc/pocketbase.env`, root-only); Shipwright wired it via systemd drop-in (`EnvironmentFile`) + restart, and shipped the Publish button page (`pb_hooks/publish_page.pb.js` → `GET /pb/publish`, "Terbitkan ke situs live"). Token permission corrected to **Contents: read+write** (the dispatch endpoint needs Contents:write, not Actions). **End-to-end verified:** a `pms-publish` repository_dispatch fires the deploy, which builds from PocketBase (`PMS_SOURCE=pb`) and deploys; live `/business /community /campus` render 7 cards each from PB. Temp test superuser deleted (`_superusers`=1), checklist `briefs`=12 intact. **PMS COMPLETE & SELF-SERVE:** edit at `/pb/_/` → click Terbitkan at `/pb/publish` → live in ~1 min. catalog.json retained as build fallback. Remaining polish (optional, later): copy the PB migration file into the repo for version control; fold the widened select values into that migration; Cloudflare purge secrets still unset (non-blocking).
